...
export const WORKFLOW_STAGES = [
  { id: 'recopilar', label: 'Recopilar' },
  { id: 'analizar', label: 'Analizar' },
  { id: 'debatir', label: 'Debatir' },
  { id: 'sintetizar', label: 'Sintetizar' },
  { id: 'checklist-riesgo', label: 'Checklist de riesgo' },
  { id: 'brief-final', label: 'Brief final' },
] as const

export const IC_STAGES = [
  { id: 'apertura', label: 'Apertura del CIO' },
  { id: 'ronda', label: 'Ronda de vistas' },
  { id: 'redteam', label: 'Desafíos (red team)' },
  { id: 'votacion', label: 'Votación formal' },
  { id: 'acta', label: 'Acta de comité' },
] as const

export const STRATEGY_STAGES = [
  { id: 'perfil', label: 'Perfil y cuota mensual' },
  { id: 'restricciones', label: 'Restricciones' },
  { id: 'asignacion', label: 'Asignación' },
  { id: 'productos', label: 'Reparto de la cuota' },
  { id: 'debate', label: 'Debate / disenso' },
  { id: 'brief', label: 'Paso a paso + brief' },
] as const
...
export async function runResearch(opts: {
  ...
  for (const s of WORKFLOW_STAGES) {
    onStage?.(s.id)
    await delay(settings.mockMode ? 220 : 100)
  }
  if (settings.mockMode || !settings.apiKey) {
    return getMockBrief(ticker, question, selectedAgents)
  }
  try {
    return await runLiveResearch({ ticker, question, selectedAgents, settings })
  } catch (e) {
    const fallback = getMockBrief(ticker, question, selectedAgents)
    fallback.synthesis.executiveSummary =
      `⚠️ Sin conexión API / fallo LLM (${(e as Error).message}). Plan local.\n\n` +
      fallback.synthesis.executiveSummary
    return fallback
  }
}
...
export async function runInvestmentCommittee(opts: {
  ...
  for (const s of IC_STAGES) {
    onStage?.(s.id)
    await delay(settings.mockMode ? 280 : 120)
  }
  if (settings.mockMode || !settings.apiKey) {
    return getMockICSession(ticker, thesis)
  }
  try {
    return await runLiveIC({ ticker, thesis, settings })
  } catch (e) {
    const fallback = getMockICSession(ticker, thesis)
    fallback.actaMemo =
      `⚠️ Sin conexión API / fallo LLM (${(e as Error).message}). Acta local.\n\n` + fallback.actaMemo
    return fallback
  }
}
...
export async function runStrategyBuilder(opts: {
  ...
  const useLive = Boolean(settings.apiKey?.trim()) && !settings.mockMode
  for (const s of STRATEGY_STAGES) {
    onStage?.(s.id)
    await delay(useLive ? 120 : 260)
  }
  if (!useLive) {
    const plan = buildMockStrategy(input)
    if (!settings.apiKey?.trim()) {
      plan.executiveBrief =
        '⚠️ Sin conexión API — plan generado con reglas locales.\n\n' + plan.executiveBrief
    }
    return plan
  }
  try {
    return await runLiveStrategy({ input, settings })
  } catch (e) {
