...
/** Only Electron main sends authenticated LLM requests. Web preview always uses the local mock. */
...
/** TODO(P4): replace this root guard with complete runtime schemas and provenance checks. */
function parseLiveObject(raw: string): Record<string, unknown> {
  let parsed: unknown
  try { parsed = JSON.parse(raw.replace(/```json|```/g, '').trim()) }
  catch { throw new Error('La respuesta del modelo no contiene JSON válido') }
...
export async function runLiveResearch(opts: {
  ...
  const parsed = parseLiveObject(await chatCompletion(opts.settings, system, user))
  return {
    ...getMockBrief(opts.ticker, opts.question, opts.selectedAgents), ...parsed,
    mode: 'live', disclaimer: DISCLAIMER, id: `brief-live-${Date.now()}`,
...
export async function runLiveIC(opts: {
  ...
  const parsed = parseLiveObject(await chatCompletion(opts.settings, system, user))
  return {
    ...getMockICSession(opts.ticker, opts.thesis), ...parsed,
    mode: 'live', disclaimer: DISCLAIMER, id: `ic-live-${Date.now()}`,
    createdAt: new Date().toISOString(),
  } as ICSession
}
...
- disclaimer corto CNMV/no bróker`
  const parsed = parseLiveObject(await chatCompletion(opts.settings, system, JSON.stringify(opts.input)))
  return {
    ...buildMockStrategy(opts.input), ...parsed,
    mode: 'live', disclaimer: DISCLAIMER, id: `strat-live-${Date.now()}`,
    createdAt: new Date().toISOString(), input: opts.input,
  } as StrategyPlan
}
